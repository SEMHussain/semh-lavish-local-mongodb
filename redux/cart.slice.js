...
const attributeDataStructure = (attr) => {
  const attrName = attr || selectedAttribute;
  const attrList = [];
  for (const k in attrName) {
    if (Object.hasOwnProperty.call(attrName, k)) {
      const el = attrName[k];
      if (el.length) attrList.push(el);
    }
  }
  return attrList.sort().toString();
};
    ...
    addVariableProductToCart: (state, action) => {
      const itemExists = state.items.find(
        (item) =>
          ...
          isEqual(item.variant, action.payload.variant)
      );
      const itemExistsWithQty = itemExists && action.payload.qty > 1;
      if (itemExistsWithQty) {
        itemExists.qty = itemExists.qty + action.payload.qty;
